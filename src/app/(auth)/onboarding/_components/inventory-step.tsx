...
const Toggle = ({
  ...
  <label className="flex cursor-pointer items-center justify-between gap-4 rounded-lg border border-[#6f5528] bg-[#3B2D16]/35 p-4 transition hover:border-[#8B6A32]">
    ...
    <span
      className={`relative h-6 w-11 shrink-0 rounded-full transition ${checked ? 'bg-[#D5AB48]' : 'bg-[#6f5528]'}`}
    >
      <input
        type="checkbox"
        checked={checked}
    ...
    <div className="space-y-7">
      <section className="rounded-xl border border-[#6f5528]/80 bg-black/10 p-4 sm:p-5">
        <div className="mb-4 flex items-start gap-3">
          ...
          <div>
            <h2 className="text-sm font-semibold text-[#F5E7C2]">
              Cigar details
            </h2>
            <p className="mt-0.5 text-xs text-[#B7A887]">
              Add the key product information customers will see.
            </p>
          </div>
        </div>
        <div className="space-y-4">
          <label className="block">
            ...
            <input
              ...
              onChange={e => onFieldChange('inventoryName', e.target.value)}
              placeholder="Padron 1964 Natural Toro"
            />
          </label>
          <div className="grid gap-4 sm:grid-cols-2">
            <label className="block">
              ...
              <input
                className={inputClassName}
                value={data.inventoryBrand}
                onChange={e => onFieldChange('inventoryBrand', e.target.value)}
                placeholder="Padron"
              />
            </label>
            <div>
            ...
            <div>
              <span className={labelClassName}>Strength</span>
              <Select
                value={data.inventoryStrength}
                onValueChange={value =>
                  onFieldChange('inventoryStrength', value)
                }
              >
                <SelectTrigger
                  aria-label="Strength"
          ...
          <label className="block">
            ...
            <textarea
              ...
              onChange={e =>
                onFieldChange('inventoryDescription', e.target.value)
              }
              placeholder="A premium handmade Nicaraguan cigar."
            />
          </label>
          <div>
      ...
      <section className="rounded-xl border border-[#6f5528]/80 bg-black/10 p-4 sm:p-5">
        <div className="mb-4 flex items-start gap-3">
          ...
          <div>
            ...
            <p className="mt-0.5 text-xs text-[#B7A887]">
              Choose where it is stored and set stock tracking values.
            </p>
          </div>
        </div>
        <div className="grid gap-4 sm:grid-cols-2">
          <label className="block">
            <span className={labelClassName}>Wall</span>
            <input className={inputClassName} value={data.wallName} disabled />
          </label>
          <div>
            <span className={labelClassName}>Shelf row</span>
            <Select
              value={data.inventoryShelfId || undefined}
              onValueChange={value => {
                const shelf = data.shelfes.find(item => item._id === value)
                onFieldChange('inventoryShelfId', value)
                onFieldChange('inventoryShelfName', shelf?.name.trim() || '')
                onFieldChange('inventoryShelfColumn', '1')
              }}
            >
              <SelectTrigger
                ...
                className={`${inputClassName} shadow-none`}
              >
                <SelectValue placeholder="Select a shelf row" />
              </SelectTrigger>
              <SelectContent className="border-[#6f5528] bg-[#2b2112] text-[#e5e1dc]">
                {data.shelfes
                  .filter(shelf => shelf._id)
                  .map(shelf => (
                    <SelectItem
                      className="focus:bg-[#4b391b] focus:text-[#f1d993]"
                      key={shelf._id}
          ...
          <label className="block">
            <span className={labelClassName}>Shelf column</span>
            <input
              className={inputClassName}
              type="number"
              min="1"
              max={Number(data.wallColumns)}
              step="1"
              inputMode="numeric"
              value={data.inventoryShelfColumn}
              onChange={e =>
                onFieldChange('inventoryShelfColumn', e.target.value)
              }
              placeholder="1"
            />
          </label>
          <label className="block">
            ...
            <input
              ...
              onChange={e => onFieldChange('inventoryQuantity', e.target.value)}
              placeholder="10"
            />
          </label>
          <label className="block">
            ...
            <input
              ...
              min="0"
              step="0.01"
              value={data.inventoryPrice}
              onChange={e => onFieldChange('inventoryPrice', e.target.value)}
              placeholder="25.99"
            />
          </label>
          <label className="block">
            <span className={labelClassName}>Low stock alert</span>
            <input
              className={inputClassName}
              type="number"
              min="0"
      ...
      <section className="rounded-xl border border-[#6f5528]/80 bg-black/10 p-4 sm:p-5">
        ...
        {image && previewUrl ? (
          <div className="overflow-hidden rounded-lg border border-[#8B6A32] bg-[#3B2D16]/35">
            ...
            <div className="flex items-center justify-between gap-3 border-t border-[#6f5528] px-4 py-3">
              <div className="min-w-0">
                <p className="truncate text-xs font-medium text-[#F5E7C2]">
                  {image.name}
                </p>
          ...
          >
            ...
            <span>
              <span className="block text-sm text-[#F5E7C2]">
                Click to upload a cigar image
              </span>
      ...
      <section className="space-y-3 rounded-xl border border-[#6f5528]/80 bg-black/10 p-4 sm:p-5">
        <div className="mb-4 flex items-start gap-3">
          ...
          <div>
            ...
            <p className="mt-0.5 text-xs text-[#B7A887]">
              Promote this product in special customer-facing sections.
            </p>
          </div>
        </div>
        <Toggle
          label="Staff Pick"
          description="Feature this cigar as a staff recommendation"
          checked={data.isStaffPick}
          onChange={value => onFieldChange('isStaffPick', value)}
        />
        {data.isStaffPick ? (
          <div className="grid gap-4 sm:grid-cols-2">
            <label className="block">
              <span className={labelClassName}>Picked by</span>
              <input
                className={inputClassName}
                value={data.staffPickBy}
                onChange={e => onFieldChange('staffPickBy', e.target.value)}
                placeholder="Mike"
              />
            </label>
            <label className="block">
              <span className={labelClassName}>Staff note</span>
              <input
                className={inputClassName}
                value={data.staffPickNote}
                onChange={e => onFieldChange('staffPickNote', e.target.value)}
                placeholder="A customer favorite"
              />
            </label>
          </div>
        ) : null}
        <Toggle
          label="New Arrival"
          description="Mark this cigar as recently added"
          checked={data.isNewArrival}
          onChange={value => onFieldChange('isNewArrival', value)}
        />
        {data.isNewArrival ? (
          <label className="block">
            <span className={labelClassName}>Arrival date</span>
            <input
              className={inputClassName}
              type="date"
        ...
        <Toggle
          ...
          description="Show this cigar in today's featured selection"
          checked={data.isDailyFeatured}
          onChange={value => onFieldChange('isDailyFeatured', value)}
        />
        {data.isDailyFeatured ? (
          <label className="block">
            <span className={labelClassName}>Featured note</span>
            <input
              className={inputClassName}
              value={data.featuredNote}
              onChange={e => onFieldChange('featuredNote', e.target.value)}
              placeholder="Try this with our new bourbon pairing"
            />
          </label>
        ) : null}
      </section>
