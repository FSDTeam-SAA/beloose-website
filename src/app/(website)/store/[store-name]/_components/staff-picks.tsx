...
import { ArrowRight, Award, RefreshCw, Sparkles } from "lucide-react";
...
function toProductCard(item: StaffPick): ProductCardData {
  return {
    id: item._id,
    name: item.name,
    brand: item.brand,
...
export default function StaffPicks() {
  ...
  return (
    <section className="bg-[#151311] py-14 text-white sm:py-20">
      <div className="container">
        <div className="mb-7 flex items-end justify-between gap-5 sm:mb-9">
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className="mt-1 text-xs text-[#9D958B] sm:text-sm">
                Personal favorites chosen by our team
              </p>
            </div>
          </div>
          ...
          {(query.data?.count || 0) > 4 && (
            <Link
              href={`/store/${encodeURIComponent(storeName)}/staff-picks`}
        ...
        {query.isError ? (
          <div className="rounded-2xl border border-red-400/20 bg-red-400/[0.06] px-5 py-10 text-center">
            ...
            <h3 className="mt-3 font-playfair text-lg text-[#F5E7D0]">
              Staff picks are unavailable
            </h3>
            <p className="mt-2 text-sm text-[#D9CFC1]">
              {query.error instanceof Error
                ? query.error.message
                : "We couldn’t load the staff picks."}
            </p>
            <button
              type="button"
          ...
          <div className="flex min-h-56 flex-col items-center justify-center rounded-2xl border border-dashed border-[#CBA24A]/25 bg-[#CBA24A]/[0.04] px-6 py-10 text-center">
            <Award className="h-8 w-8 text-[#CBA24A]" />
            <h3 className="mt-3 font-playfair text-lg text-[#F5E7D0]">
              Staff picks are coming soon
            </h3>
            <p className="mt-1 text-xs text-[#9D958B]">
              Check back soon for personal recommendations from the team.
            </p>
          </div>
        )}
      </div>
